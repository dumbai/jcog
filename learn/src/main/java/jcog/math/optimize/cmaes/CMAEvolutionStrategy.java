 ...
 *
 * <p>The main functionality is provided by the methods <code>double[][] {@link #sample()}</code> and
...
public class CMAEvolutionStrategy implements java.io.Serializable {
    ...
    static final int PARALLEL_MODE = 2;
    /**
     *
     */
    private static final String versionNumber = "0.99.40";
    private static final int mode = 0;
    private static final int SINGLE_MODE = 1; // not in use anymore, keep for later developements?
    /**
     * options that can be changed (fields can be assigned) at any time to control
     * the running behavior
     */
    public final CMAOptions options = new CMAOptions();

    public final CMAParameters param = new CMAParameters(); // alias for inside use

    /**
     * permits access to whether and which termination conditions were satisfied
     */
    public final StopCondition stop = new StopCondition();
    private final Random rand = new Random(System.currentTimeMillis());
    private final FitnessCollector fit = new FitnessCollector();
...
//    }
    ...
    private long citerlastwritten;
    private long countwritten;
    private int lockDimension;
    private long countCupdatesSinceEigenupdate;
    private double recentFunctionValue;
    private double recentMaxFunctionValue;
    private double recentMinFunctionValue;
    private int idxRecentOffspring;
    private double[][] arx;
    /**
     * recent population, no idea whether this is useful to be public
     */
    private double[][] population; // returned not as a copy
    private double[] xold;
    private double[] BDz;
    private double[] artmp;
    private String[] fileswritten = {""}; // also (re-)initialized in init()
    /**
...
//    private String propertiesFileName = "CMAEvolutionStrategy.properties";
...
//    }
    ...
    private static double[] getArrayOf(double x, int dim) {
     ...
     * @return
     */
    private static double[] assignNew(double[] rhs, double[] lhs) {
     ...
     */
    private static String getPrintAnnotation() {
        //         491( 3),   3924: 1.1245467061992267e+00 (-2e-01,4e-01)  9: 7.8e-05  2: 5.0e-02 | 5.9e-03 1.3e-02 660.41

        return "Iteration,#Fevals: rb Function Value Delta( best ,worst) |idx: Max SD idx: Min SD  | minsigD  sigma Axisratio | time, in eig";
    }
...
//    }
    ...
    private void testAndCorrectNumerics() { // not much left here
        ...
        if (iteration() > 1 || (iteration() == 1 && state >= 3))
            if (fit.fitness[0].val == fit.fitness[Math.min(param.getLambda() - 1, param.getLambda() / 2 + 1) - 1].val) {
                warning("flat fitness landscape, consider reformulation of fitness, step-size increased");
                sigma *= Math.exp(0.2 + param.getCs() / param.getDamps());
            }
        ...
        double fac = 1;
        if (MyMath.max(diagD) < 1.0e-6)
            fac = 1.0 / MyMath.max(diagD);
        else if (MyMath.min(diagD) > 1.0e4)
            fac = 1.0 / MyMath.min(diagD);
...
//     * @see #readProperties()
//     */
//	private void setFromProperties(Properties properties) {
//        String s;
//
...
//            setTypicalX(CMAOptions.parseDouble(CMAOptions.getAllToken(s)));
//        }
//        if ((s = properties.getProperty("initialX")) != null) {
//            setInitialX(CMAOptions.parseDouble(CMAOptions.getAllToken(s)));
//        }
//        if ((s = properties.getProperty("initialStandardDeviations")) != null) {
//            setInitialStandardDeviations(CMAOptions.parseDouble(CMAOptions.getAllToken(s)));
//        }
//        if ((s = properties.getProperty("dimension")) != null) { // parseInt does not ignore trailing spaces
//            setDimension(Integer.parseInt(CMAOptions.getFirstToken(s)));
//        }
////        if ((s = properties.getProperty("randomSeed")) != null) {
////            setSeed(Long.parseLong(CMAOptions.getFirstToken(s)));
////        }
//        if ((s = properties.getProperty("populationSize")) != null) {
//            sp.setPopulationSize(Integer.parseInt(CMAOptions.getFirstToken(s)));
//        }
//        if ((s = properties.getProperty("cCov")) != null) {
//            sp.setCcov(Double.parseDouble(CMAOptions.getFirstToken(s)));
//        }
//
//    }
...
//    }
     ...
     */
    private double[] init(int dimension) {
        setDimension(dimension);
        return init();
    }
    ...
    public double[] init() {
        ...
        if (state == 0) // less save variant
            return new double[param.getLambda()];
        if (state > 0)
            error("init() cannot be called after the first population was sampled");

        if (param.supplemented == 0) // a bit a hack
            param.supplementRemainders(N, options);
        param.locked = 1; // lambda cannot be changed anymore
        ...
        maxstartsigma = MyMath.max(startsigma);
        minstartsigma = MyMath.min(startsigma);
        ...
        if (xmean == null) {
            ...
            } else if (MyMath.max(UBound) < Double.MAX_VALUE
                    && MyMath.min(LBound) > -Double.MAX_VALUE) {
        ...
        if (options.diagonalCovarianceMatrix < 0) // necessary for hello world message
            options.diagonalCovarianceMatrix = 1 * 150 * N / param.lambda; // cave: duplication below
        ...
        fit.fitness = new IntDouble[param.getLambda()];   // including penalties, used yet
        fit.raw = new IntDouble[param.getLambda()];       // raw function values
        fit.history = new double[10 + 30 * N / param.getLambda()];

        arx = new double[param.getLambda()][N];
        population = new double[param.getLambda()][N];

        for (i = 0; i < param.getLambda(); ++i) {
        ...
        maxsqrtdiagC = Math.sqrt(MyMath.max(MyMath.diag(C)));
        minsqrtdiagC = Math.sqrt(MyMath.min(MyMath.diag(C)));
        ...
        clock.starteigen = System.currentTimeMillis();

        state = 0;
//    	if(CMAOptions.verbosity > -1)
//    		printlnHelloWorld();
     ...
     */
    private void eigendecomposition(int flgforce) {
        ...
        if (!flgdiag && flgforce <= 0 &&
                (clock.eigendecomposition > 1000 + options.maxTimeFractionForEigendecomposition
                        * (System.currentTimeMillis() - clock.starteigen)
                        || countCupdatesSinceEigenupdate < 1.0 / param.getCcov() / N / 5.0))
            return;
        ...
        if (flgdiag) {
            ...
            countCupdatesSinceEigenupdate = 0;
            clock.starteigen = System.currentTimeMillis(); // reset starting time
            clock.eigendecomposition = 0;             // not really necessary
        } else {
            // set B <- C
            for (i = 0; i < N; ++i)
                for (j = 0; j <= i; ++j)
            ...
            clock.eigendecomposition += System.currentTimeMillis() - firsttime;
        ...
        if (MyMath.min(diagD) == 0) // error management is done elsewhere
            axisratio = Double.POSITIVE_INFINITY;
        else
            axisratio = MyMath.max(diagD) / MyMath.min(diagD);
    ...
    */ {
        ...
        for (i = 0; i < N; ++i)
            for (j = 0; j < N; ++j) {
                ...
                if (Math.abs(cc - C[Math.max(i, j)][Math.min(i, j)]) / Math.sqrt(C[i][i] * C[j][j]) > 1.0e-10
                        && Math.abs(cc - C[Math.max(i, j)][Math.min(i, j)]) > 1.0e-9) { /* quite large */
                    s = " " + i + " " + j + " " + cc + " " + C[Math.max(i, j)][Math.min(i, j)] + " " + (cc - C[Math.max(i, j)][Math.min(i, j)]);
                    warning("cmaes_t:Eigen(): imprecise result detected " + s);
                    ++res;
                }
                if (Math.abs(dd - (i == j ? 1 : 0)) > 1.0e-10) {
                    s = i + " " + j + " " + dd;
     ...
     * @return popy
     */
    private double[][] genoPhenoTransformation(double[][] popx, double[][] popy) {
     ...
     * @return popy
     */
    private double[][] phenoGenoTransformation(double[][] popx, double[][] popy) {
        if (popy == null || popy == popx || popy.length != popx.length)
            popy = new double[popx.length][];
     ...
     * @return y
     */
    private double[] genoPhenoTransformation(double[] x, double[] y) {
        if (y == null || y == x || y.length != x.length) {
            y = x.clone();
            return y; // for now return an identical copy
        }
        if (N >= 0) System.arraycopy(x, 0, y, 0, N);
        return y;
    }
     ...
     * @return y
     */
    private double[] phenoGenoTransformation(double[] x, double[] y) {
        if (y == null || y == x || y.length != x.length) {
            y = x.clone();
            return y; // for now return an identical copy
        }
    ...
    public double[][] sample() {
        ...
        idxRecentOffspring = param.getLambda() - 1; // not really necessary at the moment
        ...
        for (iNk = 0; iNk < param.getLambda(); ++iNk) { /*
         * generate scaled
         * random vector (D * z)
         */
     ...
     */
    private double mahalanobisNorm(double[] x, double[] mean) {
        double yi, snorm = 0;
        int i, j;
     ...
     * @see #updateDistribution(double[])
     */
    private void updateDistribution(double[][] population, double[] functionValues, int nInjected) {
    ...
    public void updateDistribution(double[] functionValues) {
        ...
        counteval += lambda;
        recentFunctionValue = MyMath.min(fit.raw).val;
        recentMaxFunctionValue = MyMath.max(fit.raw).val;
        recentMinFunctionValue = MyMath.min(fit.raw).val;
        updateDistribution();
    }
    ...
    private void updateDistribution() {
        ...
        updateBestEver(arx[fit.raw[0].i], fit.raw[0].val,
                counteval - param.getLambda() + fit.raw[0].i + 1);
        ...
        if (options.diagonalCovarianceMatrix == -1) // options might have been re-read
            flgdiag = (countiter <= 150 * N / param.lambda);  // CAVE: duplication of "default"

        /* calculate xmean and BDz~N(0,C) */
        for (i = 0; i < N; ++i) {
            xold[i] = xmean[i];
            xmean[i] = 0.0;
            for (iNk = 0; iNk < param.getMu(); ++iNk)
                xmean[i] += param.getWeights()[iNk] * arx[fit.fitness[iNk].i][i];
            BDz[i] = Math.sqrt(param.getMueff()) * (xmean[i] - xold[i]) / sigma;
        }
        ...
        if (flgdiag) {
            ...
            for (i = 0; i < N; ++i) {
                ps[i] = (1.0 - param.getCs()) * ps[i]
                        + Math.sqrt(param.getCs() * (2.0 - param.getCs()))
                        * BDz[i] / diagD[i];
            }
        } else {
            /* calculate z := D^(-1) * B^(-1) * BDz into artmp, we could have stored z instead */
            ...
            for (i = 0; i < N; ++i) {
                for (j = 0, sum = 0.0; j < N; ++j)
                    sum += B[i][j] * artmp[j];
                ps[i] = (1.0 - param.getCs()) * ps[i]
                        + Math.sqrt(param.getCs() * (2.0 - param.getCs())) * sum;
            }
        }
        ...
        hsig = 0;
        if (Math.sqrt(psxps)
                / Math.sqrt(1.0 - Math.pow(1.0 - param.getCs(), 2.0 * countiter))
                / param.chiN < 1.4 + 2.0 / (N + 1.0)) {
            hsig = 1;
        }
        for (i = 0; i < N; ++i) {
            pc[i] = (1.0 - param.getCc()) * pc[i] + hsig
                    * Math.sqrt(param.getCc() * (2.0 - param.getCc())) * BDz[i];
        }

        /* stop initial phase, not in use anymore as hsig does the job */
        if (iniphase
                && countiter > Math.min(1 / param.getCs(), 1 + N / param.getMucov()))
            if (psxps / param.getDamps()
                    / (1.0 - Math.pow((1.0 - param.getCs()), countiter)) < N * 1.05)
                iniphase = false;
...
//        }
        ...
        if (param.getCcov() > 0 && !iniphase) {
            ...
            for (i = 0; i < N; ++i)
                for (j = (flgdiag ? i : 0);
                     j <= i; ++j) {
                    C[i][j] = (1 - param.getCcov(flgdiag))
                            * C[i][j]
                            + param.getCcov()
                            * (1.0 / param.getMucov())
                            * (pc[i] * pc[j] + (1 - hsig) * param.getCc()
                            * (2.0 - param.getCc()) * C[i][j]);
                    for (k = 0; k < param.getMu(); ++k) { /*
                     * additional rank mu
                     * update
                     */
                        C[i][j] += param.getCcov() * (1 - 1.0 / param.getMucov())
                                * param.getWeights()[k]
        ...
        /* update of sigma */
        sigma *= Math.exp(((Math.sqrt(psxps) / param.chiN) - 1) * param.getCs()
                / param.getDamps());
...
//    }
    ...
    private void updateBestEver(double[] x, double fitness, long eval) {
     ...
     * @see #getMeanX()
     */
    public CMASolution getBestRecentSolution() {
...
//    }
     ...
     * @see #setTypicalX(double[])
     */
    private void setTypicalX(double x) {
        if (state >= 0)
    ...
    private void setInitialStandardDeviations(double[] startsigma) {
    ...
    private String getPrintLine() {
        ...
        String s;
        if (state < 0)
            s = new PrintfFormat(Locale.US, " %4d").sprintf(countiter) +
            ...
            s = new PrintfFormat(Locale.US, " %4d").sprintf(countiter) +
                    ...
                    new PrintfFormat(Locale.US, "%2d:").sprintf(MyMath.maxidx(MyMath.diag(C))) +
                    new PrintfFormat(Locale.US, "%8.1e ").sprintf(sigma * maxsqrtdiagC) +
                    new PrintfFormat(Locale.US, "%2d:").sprintf(MyMath.minidx(MyMath.diag(C))) +
                    new PrintfFormat(Locale.US, "%8.1e ").sprintf(sigma * minsqrtdiagC) +
                    new PrintfFormat(Locale.US, "| %6.1e ").sprintf(sigma * MyMath.min(diagD)) +
                    new PrintfFormat(Locale.US, "%6.1e ").sprintf(sigma) +
                    new PrintfFormat(Locale.US, "%6.2f").sprintf(axisratio) +
                    new PrintfFormat(Locale.US, "   | %4.1f ").sprintf((System.currentTimeMillis() - clock.start) / 1000.0) +
                    new PrintfFormat(Locale.US, "%4.1f ").sprintf(clock.eigendecomposition / 1000.0);
...
//    }
    ...
    private String getDataRowFitness() {
        String s = "";
        ...
        if (mode == SINGLE_MODE)
            ...
            s += fit.raw[0].val + " ";
            s += fit.raw[param.getLambda() / 2].val + " ";
            s += fit.raw[param.getLambda() - 1].val + " ";
            s += MyMath.min(diagD) + " "
                    + (MyMath.maxidx(MyMath.diag(C)) + 1) + " " + sigma * maxsqrtdiagC + " "
                    + (MyMath.minidx(MyMath.diag(C)) + 1) + " " + sigma * minsqrtdiagC;
    ...
    private String getDataRowXRecentBest() {
        int idx = 0;
        if (mode == SINGLE_MODE)
    ...
    private String getDataRowXMean() {
        String s = "";
...
//    }
     ...
     */
    private String getDataRowAxlen() {
        String s = "";
        s = countiter + " " + counteval + " " + sigma + " " + axisratio + " "
    ...
    private String getDataRowStddev() {
        String s = "";
        s = countiter + " " + counteval + " " + sigma + " "
                + (1 + MyMath.maxidx(MyMath.diag(C))) + " " + (1 + MyMath.minidx(MyMath.diag(C))) + " ";
        for (int i = 0; i < N; ++i) {
     ...
     */
    private void writeToFile(String filename, String data, int flgAppend) {
        ...
        try (java.io.PrintWriter out = new java.io.PrintWriter(new java.io.FileWriter(filename, appendflag))) {
            out.println(data);
        ...
        if (!appendflag) {
            ...
            System.arraycopy(s, 0, fileswritten, 0, s.length);
            fileswritten[fileswritten.length - 1] = filename;
        }
    }
...
//     * @see #writeToDefaultFiles()
//     */
//    private void writeToDefaultFiles(String fileNamePrefix) {
//
...
//                return;
//            if (countiter > 4 && stop.index == 0  // has no effect if stopCondition.test() was not called
...
//                    && countiter - citerlastwritten - 1 < 2.0 * (countiter - countwritten + 1.0) / (countwritten + 1.0)
//                    // allowed time is exhausted
//                    && clock.writedefaultfiles > options.maxTimeFractionForWriteToDefaultFiles
...
//        clock.writedefaultfiles += System.currentTimeMillis() - firsttime;
...
//    }
     ...
     */
    static class MyMath { // implements java.io.Serializable {
        int itest;
        ...
        static double prod(double[] ar) {
            double res = 1.0;
            for (double v : ar) res *= v;
            return res;
        }
         ...
         */
        static int minidx(double[] ar, int maxidx) {
            int i, idx;
            idx = 0;
            for (i = 1; i < maxidx; ++i) {
                if (ar[idx] > ar[i])
         ...
         * array between index 0 and maxidx
         */
        protected static int minidx(IntDouble[] ar, int maxidx) {
            int i, idx;
            idx = 0;
            for (i = 1; i < maxidx; ++i) {
         ...
         */
        public static Double max(Double[] ar, Comparator<Double> c) {
            int i;
            Double m;
            m = ar[0];
            for (i = 1; i < ar.length; ++i) {
         ...
         */
        static IntDouble max(IntDouble[] ar) {
            int i;
            IntDouble m;
            m = ar[0];
            for (i = 1; i < ar.length; ++i) {
         ...
         */
        static IntDouble min(IntDouble[] ar) {
            int i;
            IntDouble m;
            m = ar[0];
            for (i = 1; i < ar.length; ++i) {
         ...
         */
        public static Double min(Double[] ar, Comparator<Double> c) {
            int i;
            Double m;
            m = ar[0];
            for (i = 1; i < ar.length; ++i) {
    ...
    static class Timing {
        final long birth; // time at construction, not really in use
        ...
        long writedefaultfiles;        // spent time in writeToDefaultFiles
        Timing() {
     ...
     */
    public static class CMAException extends RuntimeException {
...
//	private void writeToDefaultFilesHeaders(String fileNamePrefix, int flgAppend) {
//        if (options.maxTimeFractionForWriteToDefaultFiles < 0) // overwrites force flag
//            return;
//        String s = "(randomSeed=" + seed + ", " + new Date() + ")\n";
...
//    }
    ...
    public class StopCondition {
        int index; // number of messages collected == index where to write next message
        String[] messages = {""}; // Initialisation with empty string
        ...
        private void appendMessage(String s) {
            ...
            /* copy old messages */
            if (index >= 0) System.arraycopy(mold, 0, messages, 0, index);
        ...
        int test() {
            ...
            if ((countiter > 1 || state >= 3)
                    && Math.max(MyMath.max(fit.history), fit.fitness[fit.fitness.length - 1].val)
                    - Math.min(MyMath.min(fit.history), fit.fitness[0].val) <= options.stopTolFun)
            ...
            if (options.stopTolFunHist >= 0 && countiter > fit.history.length) {
                if (MyMath.max(fit.history) - MyMath.min(fit.history) <= options.stopTolFunHist)
            ...
            if (sigma * maxsqrtdiagC < tolx
                    && sigma * MyMath.max(MyMath.abs(pc)) < tolx)
...
class IntDouble implements Comparator<IntDouble> {
    ...
    public static boolean equals(IntDouble o1, IntDouble o2) {
