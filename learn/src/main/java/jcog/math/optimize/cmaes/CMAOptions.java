...
public class CMAOptions implements java.io.Serializable {
     ...
     */
    public long diagonalCovarianceMatrix; // -1;
    /**
     ...
     * penalties.
     */
    public double stopTolFunHist = 1.0e-13; // used if non-null
    /**
     ...
     * Default = 1e-11.
     */
    public double stopTolXFactor = 1.0e-11; // used if TolX is null
    /**
     ...
     */
    public double stopTolUpXFactor = 1.0e3; // multiplier for initial sigma
...
//    public int writeDisplayToFile = 1;
     ...
     */
    public static Double getFirstToken(String s, Double def) {
        if (s == null)
            return def;
        String[] ar = s.split("\\s+");
        if (ar[0].isEmpty())
     ...
     */
    public static Integer getFirstToken(String s, Integer def) {
        if (s == null)
            return def;
        String[] ar = s.split("\\s+");
        if (ar[0].isEmpty())
     ...
     */
    private static Long getFirstToken(String s, Long def) {
        if (s == null)
            return def;
    ...
    static String[] getAllToken(String s) {
        // split w.r.t. white spaces regexp \s+
        return removeComments(s).split("\\s+");
    }

    static double[] parseDouble(String[] ars) {
...
//            stopFitness = Double.parseDouble(getFirstToken(s));
...
//        if ((s = properties.getProperty("upperStandardDeviations")) != null && !s.isEmpty())
//            upperStandardDeviations = parseDouble(getAllToken(s));
//        if ((s = properties.getProperty("lowerStandardDeviations")) != null && !s.isEmpty())
