...
public class CMAParameters implements java.io.Serializable {
     ...
     */
    public static CMAParameters getDefaults(int N) {
        if (N == 0)
     ...
     * @see #getDefaults(int, int)
     */
    public static CMAParameters getDefaults(int N, int lambda) {
        CMAParameters p = new CMAParameters();
        p.setLambda(lambda);
        p.supplementRemainders(N, new CMAOptions());
        return p;
    ...
    private static void error(String s) { // somehow a relict from the C history of this code
        ...
        throw new CMAEvolutionStrategy.CMAException(" CMA-ES error: " + s); // TODO this looks like a real hack
        //      System.exit(-1);
    }
            ...
            damps =
                    (1 + 2 * Math.max(0, Math.sqrt((mueff - 1.0) / (N + 1.0)) - 1))
                            * Math.max(0.3, 1 -                         /* modification for short runs */
                            N / (1.0e-6 + Math.min(opts.stopMaxIter,
...
//	int flgLambdaChanged; // not in use yet

    /**
     * Setter for parent number mu, be aware of the recombinationType when setting mu
     *
     * @param mu New value for the number of parents mu.
...
//		assert locked == 0;
     ...
     * Setter for mucov.
     *
     * @param mucov New value of mucov.
     * @see #getMucov()
     */
     ...
     * Setter for damps.
     *
     * @param damps New value of damps.
     * @see #getDamps()
     */
